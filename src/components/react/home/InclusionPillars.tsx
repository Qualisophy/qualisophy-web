...
interface InclusionPillarsProps {
  data: any;
}

export const InclusionPillars: React.FC<InclusionPillarsProps> = ({ data }) => {
  const [activeId, setActiveId] = useState<string>("neurodivergence");

  // Mezclamos la configuración visual fija con los textos que vienen del TS
  const visualConfig = [
    {
      id: "neurodivergence",
      icon: "psychology",
    ...
    <section className="bg-gray-50 py-16 w-full border-t border-gray-100">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="flex flex-col md:flex-row h-[600px] md:h-[500px] lg:h-[600px] w-full gap-2 md:gap-4 items-stretch">
          {data.items.map((pillar: any, index: number) => {
